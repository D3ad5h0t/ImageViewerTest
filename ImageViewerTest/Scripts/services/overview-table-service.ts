class OverviewTable {
    ...
    public init(tableId: string, paginationId: string) {
        ...
        OverviewTable.pagination = document.querySelector(`#${paginationId}`) as HTMLElement;
    }
    ...
    private setVisibleNearestPages(pageInfo: PageInfo) {
        ...
        if (currentPage) {
            ...
            if (pageInfo.pageNumber > 1 && pageInfo.pageNumber < pageInfo.totalPages) {
                OverviewTable.pagination.querySelector(`.page-item-${pageInfo.pageNumber + 1}`)?.classList.remove("hidden-item");
                OverviewTable.pagination.querySelector(`.page-item-${pageInfo.pageNumber - 1}`)?.classList.remove("hidden-item");
            } else if (pageInfo.pageNumber == 1) {
                OverviewTable.pagination.querySelector(`.page-item-${pageInfo.pageNumber + 1}`)?.classList.remove("hidden-item");
                OverviewTable.pagination.querySelector(`.page-item-${pageInfo.pageNumber + 2}`)?.classList.remove("hidden-item");
